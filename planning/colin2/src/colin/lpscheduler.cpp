                    ...
                    for (; invItr != invEnd; ++invItr) {
                        ...
                        for (int s = 0 ; s < cSize; ++s) {
                            if ((*invItr)->variables[s] < 0) {
                                ...
                                entries.push_back(make_pair(finalNumericVars[(*invItr)->variables[s]].lastEffectValueVariable, (*invItr)->weights[s]));
                                if (assertions) assert(entries.back().second != 0.0);
                            }
                        }

                        lp->addRow(entries,
                                   ((*invItr)->lower != -DBL_MAX ? (*invItr)->lower : -LPinfinity),
                                   ((*invItr)->upper != DBL_MAX  ? (*invItr)->upper : LPinfinity ) );
