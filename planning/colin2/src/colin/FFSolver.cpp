    ...
    if (considerCache) {
        if (FFcache_upToDate) {
            ...
            h = RPGBuilder::getHeuristic()->getRelaxedPlan(theState.getInnerState(), &(theState.startEventQueue), minTimestamps, theState.timeStamp,
                                                           ...
                                                           helpfulActions, relaxedPlan, makespanEstimate, justApplied, tilFrom, rplan);
...
map<double, ActionLevel> FF::get_relaxed_plan()
{
    static bool initCSBase = false;
...
//};
    ...
    map<double, ActionLevel> to_return;

    list<pair<double, list<ActionSegment> > > rp = generateRelaxedPlan(initialState, 0, goals, numericGoals, 
      (ParentData*) 0, initialSQI->helpfulActions, initialSQI->plan, tEvent, -1, &to_return);

    for(list<pair<double, list<ActionSegment> > >::const_reference e : rp)
    {
