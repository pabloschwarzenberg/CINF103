...
void RPGBuilder::removePointlessEffects()
{
    ...
    LiteralSet::const_iterator fItr = factsWithOnlyPointlessEffects.begin();
    const LiteralSet::const_iterator fEnd = factsWithOnlyPointlessEffects.end();
    ...
    for (; fItr != fEnd; ++fItr) {
        ...
        for (pointless_effect eff = PE_ADDED; eff < PE_DELETED_THEN_ADDED; eff = (eff == PE_ADDED ? PE_DELETED : PE_DELETED_THEN_ADDED)) {
           ...
           if (Globals::globalVerbosity & 16384) {
                if (!eta.empty()) {
                    if (eff == PE_ADDED) {                
                        cout << "Add effects on " << *(*fItr) << " are pointless\n";
                    } else {
